...
<main class=" font-dusha mt-24 bg-[#F4F7F6]">
    ...
    <form enctype="multipart/form-data" action="{{ route('dtks.store') }}" method="POST" class="mx-auto my-10 w-11/12 max-w-prose p-4 flex flex-col gap-4 font-sans" id="myForm">
        @csrf
        ...
        <!-- rt/rw -->
        <div class="bg-white p-4 w-full rounded-md">
            <label for="name">RT / RW <span class="text-[#FF0000]">*</span> <br>
                <input type="text" name="rt/rw" id="name" class="w-full mt-2 outline-none border-b-2 border-black border-dotted focus:border-solid" placeholder="RT / RW" required>
            </label>
        </div>
        ...
        <div class="bg-white p-4 w-full rounded-md">
            <div class="flex justify-between items-center mb-2">
                <span>Unggahan Gambar: <span class="upload-counter">0</span>/4</span>
            </div>
        ...
        <!-- foto rumah -->
        <div class="bg-white p-4 w-full rounded-md">
            <span>Foto Rumah <span class="text-[#FF0000]">*</span></span>
            <!-- File Upload Item 3 -->
            <div class="file-upload-item mt-4">
                <!-- Preview Gambar -->
                <div class="mb-3">
                ...
                <div class="mb-2">
                    <div class="flex items-center">
                        <label class="file-label cursor-pointer bg-blue-500 hover:bg-blue-600 text-white text-sm font-medium py-1.5 px-3 rounded-lg mr-2 transition-colors duration-200">
                            Pilih File
                            <input type="file" name="image-rumah" accept="image/jpeg,image/png,image/gif,image/webp" class="image-upload hidden" required />
                        </label>
                        <span class="file-name text-xs text-gray-500 truncate max-w-[150px]">Belum ada file</span>
                    </div>
                    <p class="mt-1 text-xs text-gray-500">Maks. 5MB</p>
